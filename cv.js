...
(() => {
  ...
  if (skillsArea) {
      ...
      new IntersectionObserver(
        entries => {
          if (entries[0].isIntersecting) {
            skillsArea.classList.add('in-view');
    ...
    skillObserver.observe(skillsArea);
  }
  ...
  // =========================================================
  // FLOWSTATE WEBGL FLUID BACKGROUND
  // =========================================================
  ...
  function fluidSimulation(canvas) {
    ...
    function render(target) {
      ...
      if (config.SHADING) {
        ...
        gl.uniform1i(
          displayShadingProgram.uniforms.uTexture,
...
/* ---------------------------------------------------------------------- */
/* Smooth scroll (kept for parity — the page is a single inert screen)    */
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- */
function revealBlock(el, delay, fromTransform) {
  el.style.opacity = '0';
  el.style.transform = fromTransform;
  el.style.transition = 'opacity 700ms var(--ease-entrance), transform 700ms var(--ease-entrance)';
  setTimeout(() => {
    el.style.opacity = '1';
...
function splitWords(el, { fromY, stagger, duration, baseDelay }) {
  const easing = 'cubic-bezier(0.33, 1, 0.68, 1)'; /* easeOutCubic approximation */
...
function initReveals() {
  revealBlock(document.querySelector('.nav'), 150, 'translateY(-0.75rem)');
  revealBlock(document.getElementById('badge'), 320, 'translateY(1.25rem)');

  splitWords(document.getElementById('heading'), {
    fromY: 26, stagger: 85, duration: 720, baseDelay: 480
  });
  splitWords(document.getElementById('subline'), {
    fromY: 14, stagger: 22, duration: 600, baseDelay: 1150
  });

  revealBlock(document.getElementById('form-wrap'), 1450, 'translateY(1.25rem)');
  revealBlock(document.getElementById('footer'), 1650, 'translateY(1.25rem)');
}

initReveals();

/* ---------------------------------------------------------------------- */
/* WebGL fluid simulation — oily ink-in-water background                  */
/* (Pavel Dobryakov's WebGL fluid sim, MIT — tuned config + load burst +  */
/* invisible perpetual auto-cursor, per spec, verbatim otherwise)         */
/* ---------------------------------------------------------------------- */
function fluidSimulation(canvas) {
    canvas.width = canvas.clientWidth;
    canvas.height = canvas.clientHeight;
...
const fluidCanvas = document.querySelector('canvas');
if (fluidCanvas) fluidSimulation(fluidCanvas);
